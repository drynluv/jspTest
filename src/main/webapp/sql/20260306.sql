...
-- students테이블의 나이가 21살 이상 23살 이하인 학생을 보여주시오?(between ~ and)
...
-- 앞의 결과에 천단위 쉼표표시, 소수이상 버리기(format())
...
-- 부서별 평균급여를 출력하되 평균급여가 1000원 이상인 자료 중 '기획'부서 외의 자료만 출력?
select buseo, format(avg(pay), 0) as 평균급여 from jikwon where buseo!= '기획' group by buseo having avg(pay) >= 1000;

-- 부서별 평균급여를 출력하되 평균급여가 1000원 이상인 자료 중 '기획'부서 외의 자료만 평균급여 내림차순 출력?
...
-- 앞의 경우처럼 참조하는 필드명이 같을 경우 using절로 대처할 수 있다.
...
-- 인라인 뷰(view) : 뷰의 장점:논리적 형태로 실제 존재하지 않음. 보안성(노출안됨), 독립성(구조변경시 일관성 유지), 편의성(별명사용시 쿼리를 단순화)
...
-- 사원테이블에서 같은 부서의 월급이 해당 부서의 최고 월급보다 작은 사람들을 조회해보자.(성명, 급여)
...
select *, rank() over (order by jumsu desc) as 순위 from sungjuks;
...
select *, LAG(mechul) over (PARTITION BY jikwon order by nalja) from panme2;
select *, LAG(mechul) over (order by jikwon) from panme2;
...
select *, LAG(mechul) over (partition by jikwon order by nalja) as 다음매출 from panme2;
...
select *, FIRST_VALUE(mechul) over (partition by jikwon order BY nalja ROWS BETWEEN unbounded preceding AND unbounded following) as 마지막매출 from panme2;
