...
<div class="container">
	...
	<div>
		...
		1-3. 길이(vos객체길이) : ${fn:length(vos)}<br/> <%-- vos.size() --%>
		<br/>
		...
		<pre> (과제)
				...
				- message변수 안에 들어있는 'o'의 전체 개수와 위치는?(forEach활용....)
				
		</pre>
		<%--
				검색문자열의 뒤쪽 전체 자료 추출 : substringAfter()
				검색문자열의 앞쪽 전체 자료 추출 : substringBefore()
		 --%>
		
		4-2.- message 변수 내용 중 두번째 'o'의 위치?substringAfter()
					${fn:indexOf(message, fn:substringAfter(fn:substringAfter(message, 'o'), 'o'))-1}<br/>
		4-3.- substringBefore() : ${fn:substringBefore(message, 'o')}<br/>
		<br/>
		5-1.문자열분리(split(변수,분리할 문자)) : 'o'문자를 기준으로 각각을 분리시켜 출력?<br/>
		<c:set var="strs" value="${fn:split(message, 'o')}"/>
		<c:forEach var="str" items="${strs}" varStatus="st">
			${st.count}. ${str}<br/>		
		</c:forEach>
		message 변수 안의 'o'문자의 개수는? ${fn:length(strs)-1} 개<br/>
		<br/>
		6.치환(replace())<br/>
		str변수 안의 'o'를 'O'로 치환? ${fn:replace(message, 'o', 'O')}<br/>
		<br/>
		7-1. 포함(contains()) : <br/>
