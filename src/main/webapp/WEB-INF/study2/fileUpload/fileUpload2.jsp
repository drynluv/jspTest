...
<head>
  ...
  function fCheck()	{
	  ...
	  let fName3 = document.getElementById("file3").value;
		...
		else if(ext1 != 'jpg' && ext1 != 'gif' && ext != 'png' && ext1 != 'zip' && ext1 != 'hwp' && ext1 != 'ppt' && ext1 != 'xls' && ext1 != 'pdf' && ext1 != 'txt') {
		...
		else if(ext3 != 'jpg' && ext3 != 'gif' && ext3 != 'png' && ext3 != 'zip' && ext3 != 'hwp' && ext3 != 'ppt' && ext3 != 'xls' && ext3 != 'pdf' && ext3 != 'txt') {
			alert("업로드 가능파일은 'jpg/gif/png/zip/hwp/ppt/xls/pdf/txt'입니다.");
		}
		else {
...
<div class="container">
  ...
  <form name="myform" method="post" action="FileUpload2Ok.st" enctype="multipart/form-data">
    <div>
	    <input type="file" name="fName1" id="file1" class="form-control mb-1" accept=".jpg,.gif,.png,.xls,.zip,.ppt,.doc,txt"/><br/>
	    <input type="file" name="fName2" id="file2" class="form-control mb-1" accept=".jpg,.gif,.png,.xls,.zip,.ppt,.doc,txt"/><br/>
	    <input type="file" name="fName3" id="file3" class="form-control mb-1" accept=".jpg,.gif,.png,.xls,.zip,.ppt,.doc,txt"/><br/>
