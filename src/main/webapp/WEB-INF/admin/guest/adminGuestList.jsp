...
<div class="container">
	...
	<table class="table table-borderless m-0 p-0">
		<tr>
			<td>번호 : ${curScrStartNo} &nbsp;
				<a href="javascript:deleteCheck(${vo.idx})" class="badge bg-danger">삭제</a></td>
	...
	<table class="table table-bordered">
		...
		<tr>
			<th>메일주소</th>
			<td colspan="3">
				<c:if test="${empty vo.email || fn:length(vo.email)<=6 || fn:indexOf(vo.email,'.')==-1 || fn:indexOf(vo.email,'@')==-1 || fn:indexOf(vo.email,'.')==-1}">- 없음 -</c:if>
				<c:if test="${!empty vo.email && fn:length(vo.email)>6 && fn:indexOf(vo.email,'.')!=-1 && fn:indexOf(vo.email,'@')!=-1 && fn:indexOf(vo.email,'.')!=-1}">${vo.email}</c:if>
			</td>
		</tr>
		<tr>
			<th>홈페이지</th>
...
<ul class="pagination">
	...
	<c:if test="${pag > 1}">
	<li class="page-item">
		<a class="page-link" href="AdminGuestList.ad?pag=1">«</a>
	</li>
	</c:if>
	...
	<li class="page-item">
		<a class="page-link" href="AdminGuestList.ad?pag=${(curBlock-1)*blockSize+1}">‹</a>
	</li>
	</c:if>
	...
	<c:otherwise>
	<li class="page-item">
		<a class="page-link" href="AdminGuestList.ad?pag=${i}">${i}</a>
	</li>
	</c:otherwise>
	...
	<c:if test="${pag < totPage}">
	<li class="page-item">
		<a class="page-link" href="AdminGuestList.ad?pag=${totPage}">»</a>
	</li>
	</c:if>
